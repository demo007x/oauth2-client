...
func TestNewOauthRefreshToken(t *testing.T) {
	...
	token := NewOauthRefreshToken(serverURL, key, secret, refreshToken, OauthRefreshTokenWithGrantType(grantType), RefreshTokenWithResponseHandler(func(resp *http.Response) ([]byte, error) {
		defer func() {
			resp.Body.Close()
		}()
